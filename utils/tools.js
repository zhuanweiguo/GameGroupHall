...
export function giveAvatar(item) {
	return item.user_id[0]?.avatar_file?.url ?? '../../static/myIcon/user.png'
}
...
//加入操作数据库的方法
export async function joinFun(artid) {
	let count = await db.collection("team_join")
		.where(`article_id=="${artid}" && user_id==$cloudEnv_uid`).count()
	if (count.result.total) {
		db.collection("team_join").where(`article_id=="${artid}" && user_id==$cloudEnv_uid`)
			.remove();
		utilsObj.operation("team_article", "join_count", artid, -1)

	} else {
		db.collection("team_join").add({
			article_id: artid
		})
		utilsObj.operation("team_article", "join_count", artid, 1)
	}
...
function getIp() {
	return new Promise((resolve, reject) => {
		uni.request({
			...
			success: res => {
				let str = ""
				typeof(res.data.province) == "string" ? str = res.data.province: str = "未知地"
				resolve(str)
				let obj = {
					province: str,
					time: Date.now()
